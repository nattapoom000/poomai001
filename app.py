...
st.write("แอปพลิเคชันสร้างเฉลย Google Form (เวอร์ชันเชื่อมโยงเวลาจริงประเทศไทย 🇹🇭)")
...
if st.button("🚀 ให้ POOM AI เริ่มสแกนข้อสอบ", type="primary"):
    if not api_key_input or not form_url.startswith("http"):
        ...
        try:
            ...
            export_text = f"เฉลยข้อสอบจาก POOM AI SNTC V6\nเวลาอ้างอิง: {current_datetime_th}\n" + ("="*40) + "\n\n"
            ...
            for index, block in enumerate(question_blocks):
                ...
                if index > 0:
                    time.sleep(5) # เพิ่มเวลาหน่วงเป็น 5 วินาที เพื่อป้องกันข้อจำกัดโควตา (Rate Limit)
...
คำสั่ง: ตอบคำถามนี้ด้วยข้อความสั้นๆ กระชับ ได้ใจความ และถูกต้องที่สุด ห้ามพิมพ์คำอธิบายหรือคำเกริ่นนำใดๆ ตอบเฉพาะส่วนที่เป็นคำตอบเท่านั้น"""
                ...
                # ส่งถาม AI พร้อมระบบ Retry ป้องกันลิมิต
                ai_answer = ""
                for attempt in range(1, 4):
                    try:
                        ...
                        if "429" in str(ai_err) or "quota" in str(ai_err).lower():
                            st.warning(f"⏳ AI ติดโควตา... ระบบกำลังรอ 20 วินาทีเพื่อรีเซ็ต (ครั้งที่ {attempt}/3)")
                            time.sleep(20)
                        else:
