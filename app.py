...
with st.expander("⚙️ การตั้งค่าระบบและเลือกโหมดการทำงาน (คลิกเพื่อซ่อน/แสดง)", expanded=True):
    ...
    # แสดงช่องกรอก API Key เฉพาะตอนที่ต้องใช้ AI
    if "AI สแกน" in app_mode:
        ...
        api_key_input = "bypass_key_for_screenshot_mode" # ไม่จำเป็นต้องใช้ AI Key ในโหมดแคปจอเพียวๆ
        ai_provider = "None"
...
if st.button("🚀 เริ่มการทำงานของระบบ", type="primary"):
    if not form_url.startswith("http") or (not api_key_input and "AI สแกน" in app_mode):
        ...
        # ตั้งค่า AI Client (กรณีเลือกโหมด AI)
        ai_client = None
        openai_client = None
        working_model_name = ""
        ...
        try:
            ...
            # ขยายขนาดหน้าต่างเบราว์เซอร์ให้กว้างและสูง เพื่อความคมชัดสูงสุดในการแคปจอ
            ...
            # --- ระบบข้ามหน้าแรก (กรอกชื่อและกดถัดไปเพื่อให้เห็นข้อสอบ) ---
            ...
            if app_mode == "📸 เมนูแคปจอออโต้: แคปหน้าจอทั้งหมด 1 ภาพเดี่ยว":
                ...
                # คำนวณความสูงหน้าเว็บทั้งหมดเพื่อให้แคปได้ครบถ้วน ชัดเจนที่สุด
            ...
            # ==========================================
                ...
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                    for index, block in enumerate(question_blocks):
                        ...
                        try:
                            # บันทึกภาพย่อยเฉพาะบล็อกข้อสอบนั้นๆ ด้วยความคมชัดสูงสุด
                            ...
                            # เพิ่มภาพลงในไฟล์ ZIP อัตโนมัติ
                ...
                # ปุ่มดาวน์โหลดไฟล์ ZIP รวมภาพทุกข้อ
                st.download_button(
            ...
            # ==========================================
            # โหมดที่ 3: AI สแกนหาเฉลยข้อสอบ (ระบบเดิม)
            # ==========================================
