...
st.write("แอปพลิเคชันสร้างเฉลย Google Form (รองรับหลายค่าย AI 🇹🇭)")

# ==========================================
# ส่วนที่ 1: รับค่าและการตั้งค่าระบบ
# ==========================================
with st.expander("⚙️ การตั้งค่าระบบและเลือก AI (คลิกเพื่อซ่อน/แสดง)", expanded=True):
...
if st.button("🚀 ให้ POOM AI เริ่มสแกนข้อสอบ", type="primary"):
    if not api_key_input or not form_url.startswith("http"):
        st.error("❌ กรุณาใส่ API Key และลิงก์ Google Form ให้ถูกต้อง")
    else:
        st.info(f"🌐 กำลังเชื่อมต่อระบบ AI ({ai_provider}) และเปิดบอทสแกนเนอร์...")
        
        # ตรวจสอบและตั้งค่าตัวเชื่อมต่อตามค่ายที่เลือก
        ai_client = None
        openai_client = None
        ...
        try:
            if ai_provider == "Google Gemini":
                ...
                st.success(f"✅ เชื่อมต่อ Gemini สำเร็จ! (โมเดล: {working_model_name})")
                
            elif ai_provider == "OpenAI (ChatGPT)":
                openai_client = OpenAI(api_key=api_key_input)
                working_model_name = 'gpt-4o-mini' # หรือใช้ gpt-4o
                st.success(f"✅ เชื่อมต่อ OpenAI สำเร็จ! (โมเดล: {working_model_name})")
        ...
        try:
            ...
            # --- 3. กวาดข้อสอบ (วิเคราะห์และสรุปคำตอบ) ---
            question_blocks = driver.find_elements(By.XPATH, "//div[@role='listitem']")
            st.success(f"พบบล็อกคำถามทั้งหมด {len(question_blocks)} ข้อ (ตั้งค่าเวลาอ้างอิง: {current_datetime_th})")
            ...
            for index, block in enumerate(question_blocks):
                ...
                if index > 0:
                    time.sleep(4) # หน่วงเวลาระหว่างข้อ
...
คำสั่ง: ตอบคำถามนี้ด้วยข้อความสั้นๆ กระชับ ได้ใจความ และถูกต้องที่สุด ห้ามพิมพ์คำอธิบายหรือคำเกริ่นนำใดๆ ตอบเฉพาะส่วนที่เป็นคำตอบเท่านั้น"""
                ...
                # ส่งถาม AI ตามค่ายที่ผู้ใช้เลือก
                ai_answer = ""
                for attempt in range(1, 4):
                    try:
                        if ai_provider == "Google Gemini":
                            ...
                            # แปลงภาพเป็น Base64 สำหรับส่งให้ OpenAI มองเห็นภาพ
                            import base64
                            buffered = BytesIO()
                            if block_img:
                        ...
                        if "429" in str(ai_err) or "quota" in str(ai_err).lower() or "rate_limit" in str(ai_err).lower():
                            st.warning(f"⏳ AI ติดโควตา/Rate Limit... กำลังรอ 15 วินาที (ครั้งที่ {attempt}/3)")
                            time.sleep(15)
                        else:
